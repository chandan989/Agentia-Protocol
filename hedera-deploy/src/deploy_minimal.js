...
function compileContract(contractPath) {
  ...
  if (!artifact?.evm?.bytecode?.object) throw new Error('Compilation failed or bytecode missing');
  return { bytecodeHex: artifact.evm.bytecode.object.replace(/^0x/, ''), contractName };
}
...
(async () => {
  ...
  const contractPath = path.resolve(__dirname, '../contracts/TestDeploy.sol');
  ...
  try {
    ...
    console.log(`Minimal contract deployed: ${contractId}`);
  } catch (err) {
    const status = err?.status?.toString?.() || 'UNKNOWN';
    console.error('Minimal deploy failed with status:', status);
